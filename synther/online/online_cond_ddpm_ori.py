...
def redq_sac(
    ...
    for t in range(total_steps):
        ...
        # retrain lands on the final logging step: prior training + RTB fine-tuning +
        # `num_samples` generation run (~30 min) and then the run ends, with the epoch's
        # metrics still reflecting the *previous* buffer. Pure waste.
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Ours: PGR with RTB posterior fine-tuning of the DDPM prior. '
                    'Per-task values (gin file, cond_top_frac, alpha_rtb, ...) are '
                    'resolved from synther/online/env_defaults.py, so a run normally '
                    'only needs --env / --novelty_measure / --seed / --wandb.')
    ...
    # None -> 150 for quadruped-walk-v0, else 100.
    parser.add_argument('--num_posterior_epochs', type=int, default=None)
    ...
    # The three below are None -> resolved per (env, novelty_measure). See env_defaults.py.
    parser.add_argument('--ft_clip_grad', type=float, default=None)
    parser.add_argument('--alpha_rtb', type=float, default=None)
    ...
    apply_env_defaults(args, rtb=True)
