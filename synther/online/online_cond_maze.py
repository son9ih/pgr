...
from redq.algos.core_maze import mbpo_epoches, test_agent
from redq.utils.bias_utils_maze import log_bias_evaluation
...
def redq_sac(
    ...
    env_fn = lambda: wrap_gym_maze(gym.make(env_name))
    ...
    for t in range(total_steps):
        ...
        # - If goal is reached (terminated=True from MazeCustomWrapper), episode ends
        # - If max_ep_len is reached (truncated or artificial horizon), episode ends
        # - Store the done signal properly for learning
        ep_len += 1
        
        # For PointMaze, keep done signal if goal was reached (terminated)
        # Only ignore done if it's from max_ep_len (time limit)
        if terminated:
        ...
        # Note: Episode reset is now handled above after each step
...
def wrap_gym_maze(env: gym.Env, rescale_actions: bool = True) -> gym.Env:
    """
    Wrap PointMaze environment with custom initial state and goal rewards.
    - Initial state: Center of the maze
    - Goal states: 4 corners/edges with rewards [2, 3, 4, 5]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='PointMaze_Medium-v3')
