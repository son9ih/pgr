...
def redq_sac(
    ...
    if args.wandb:
        if args.synther:
            ...
            if args.sac:
                run_name = f"{env_name}_{seed}_{time.strftime('%Y%m%d-%H%M%S')}_SAC"
                group_name = 'SAC'
            elif args.redq:
                run_name = f"{env_name}_{seed}_{time.strftime('%Y%m%d-%H%M%S')}_REDQ"
                group_name = 'REDQ'
            else:
    ...
    for t in range(total_steps):
        ...
        if not disable_diffusion and (t + 1) % retrain_diffusion_every == 0 and (t + 1) >= diffusion_start:
            ...
            if args.novelty_measure == 'curiosity':
                # 이걸 args.cond_top_frac 대신 gin에 있는 cond_top_frac를 사용하도록 수정해야 함
                # 지금은 그냥 스크립트에서 두개 다 수정
