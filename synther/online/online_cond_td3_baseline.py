...
from synther.online.td3_rlpd_agent import TD3RLPDCondAgent

import wandb
from synther.online.utils import PBE, RMS, compute_intr_reward
import pdb

from synther.diffusion.diffusion_cond import DiffusionModel, QFlow
...
def td3_training(
        ...
        n_evals_per_epoch=1,
        logger_kwargs=dict(),
        # TD3 agent hyperparameters (from rlkit/torch/td3)
    ...
    if args.wandb:
        if args.synther:
            run_name = f"{env_name}_{seed}_{time.strftime('%Y%m%d-%H%M%S')}_TD3_SER_DDPM"
            group_name = 'TD3_SER_DDPM'
        else:
            run_name = f"{env_name}_{seed}_{time.strftime('%Y%m%d-%H%M%S')}_TD3_PGR+{args.novelty_measure}_DDPM"
            group_name = f'TD3_PGR+{args.novelty_measure}_DDPM'
        wandb.init(
            entity="gda-for-orl",
            project = env_name,
            group = group_name,
    ...
    """init TD3 agent + buffer and start training"""
    agent_config = {
        'env_name': env_name,
        'cond_hidden_size': cond_hidden_size,
    ...
    for t in range(total_steps):
        ...
        # +++++++++++ training over +++++++++++
            ...
            # +++++++++++ 2. Sampling (baseline: prior only, no QFlow) +++++++++++
            print(f'Sampling...')
            X_sample_total = []
        ...
        if (t + 1) % steps_per_epoch == 0:
            ...
            if evaluate_bias:
                # TD3 uses deterministic policy; alpha=0 for bias evaluation (no entropy term)
                log_bias_evaluation(bias_eval_env, agent, logger, max_ep_len, alpha=0.0, gamma=gamma, n_mc_eval=n_mc_eval, n_mc_cutoff=n_mc_cutoff)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gin_config_files', nargs='*', type=str,
                        default=['config/online/td3_cond_synther_dmc.gin'])
    ...
    if args.synther:
        args.results_folder = f"/network/scratch/y/yunt/pgr/results/{args.env}_{args.seed}_{args.novelty_measure}_SER"
    else:
        args.results_folder = f"/network/scratch/y/yunt/pgr/results/{args.env}_{args.seed}_{args.novelty_measure}_PGR"
    print(args.results_folder)
    ...
    td3_training(args.env, logger_kwargs=logger_kwargs, args=args)
