...
def td3_training(
    ...
    if args.wandb:
        run_name = f"TD3_{env_name}_{seed}_{time.strftime('%Y%m%d-%H%M%S')}_Ours+{args.novelty_measure}_ftlr{args.finetune_lr}_clip{args.ft_clip_grad}_A{args.alpha_rtb}_On{args.inter_onpolicy}_anl{args.anneal}"
        wandb.init(
            entity="gda-for-orl",
            project = env_name,
            group = f'TD3_Ours+{args.novelty_measure}',
            name = run_name,
            config={
                "env_name": env_name,
                "algorithm": "TD3",
