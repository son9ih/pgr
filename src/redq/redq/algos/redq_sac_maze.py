...
from redq.algos.core_maze import TanhGaussianPolicy, Mlp, soft_update_model1_with_model2, ReplayBuffer,\
...
class REDQSACAgent(object):
    ...
    def __init__(self, env_name, obs_dim, act_dim, act_limit, device,
                 ...
                 policy_update_delay=20, rnd=False
                 ):
        # set up networks
    ...
    def sample_data_recent(self, batch_size):
        ...
        if ptr_location >= batch_size:
            idxs = np.arange(ptr_location-batch_size, ptr_location)
        else:
            idxs1 = np.arange(0, ptr_location)
            idxs2 = np.arange(self.replay_buffer.max_size-(batch_size-ptr_location), self.replay_buffer.max_size)
    ...
    def sample_diffusion_data(self, batch_size):
        # sample data from diffusion buffer
    ...
    def sample_diffusion_data_cpu(self, batch_size):
        # sample data from diffusion buffer, return cpu tensors
        batch = self.diffusion_buffer.sample_batch(batch_size)
        obs_tensor = Tensor(batch['obs1'])
        obs_next_tensor = Tensor(batch['obs2'])
        acts_tensor = Tensor(batch['acts'])
