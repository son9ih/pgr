...
| [tools/plot_buffer_ablation.py](tools/plot_buffer_ablation.py) | 위 CSV → 3곡선 figure (`figures/`) |
...
실행은 [04_script.md](04_script.md) 참고. 요약:

```bash
bash scripts/run.sh ours hopper           # 런처 (권장)
python synther/online/online_cond_ddpm_ori.py --env hopper --seed 0 --wandb
```

`.py`를 직접 부를 때는 반드시 `python <script>.py ...` 형태로 한다. 스크립트에 실행 비트도
shebang도 없어서 `./synther/online/...py`로 부르면 `Permission denied`가 난다.
